...
import {
  IValidationErrFormat,
  ValidationErr,
} from '../../src/common/route-errors';

import Paths from '../support/Paths';
import { convertValidDates } from '../support';
import { TApiCb, TRes } from '../types/misc';
...
describe('UserRouter', () => {
  ...
  describe(`"GET:${Paths.Users.Get}"`, () => {
    // Setup API
    const api = (cb: TApiCb) => agent.get(Paths.Users.Get).end(wrapCb(cb));
  ...
  describe(`"POST:${Paths.Users.Add}"`, () => {
    ...
    const callApi = (user: IUser | null, cb: TApiCb) =>
      agent.post(Paths.Users.Add).send({ user }).end(wrapCb(cb));
  ...
  describe(`"PUT:${Paths.Users.Update}"`, () => {
    ...
    // Setup API
    const callApi = (user: IUser | null, cb: TApiCb) =>
      agent.put(Paths.Users.Update).send({ user }).end(wrapCb(cb));
  ...
  describe(`"DELETE:${Paths.Users.Delete}"`, () => {
    ...
    const callApi = (id: number, cb: TApiCb) =>
      agent.delete(insertUrlParams(Paths.Users.Delete, { id })).end(wrapCb(cb));
